...
### Faster algorithm for the saturated MLE of BvNB2

ML.BvNB2.sat <- function(xvec, yvec) {
  xy.reduced <- reduce(xvec, yvec)
  xy.sat.lik <- apply(xy.reduced, 1, function(x) {
    mle <- ML.BvNB2(x[1], x[2])
    return(lik.BvNB2(x[1], x[2], param=mle))
  })
  sat.lik <- sum(xy.reduced$freq * xy.sat.lik)
  return(sat.lik)
}